...
## Session 3: Port node-banana Batch 2 (9 features)
...
Ported 9 remaining pure-frontend features from node-banana to workflow-ui OSS core package. Created 8 new files, modified 9 existing files. All 211 existing tests pass; zero new TypeScript errors.

#### Feature 1: Extended Easing Library
- Extended `lib/easing.ts` with 22 parametric functions (quad, cubic, quart, quint, sine, expo, circ — In/Out/InOut), 2 hybrid asymmetric easings
- Added `createAsymmetricEase`/`createBezierEasing` factories, `getEasingFunction`/`getAllEasingNames` lookups
- Full `EASING_BEZIER_MAP` with 20+ bezier approximations, `PRESET_BEZIERS` for CubicBezierEditor, `DEFAULT_CUSTOM_BEZIER`

#### Feature 2: Speed Curve Utility
- `lib/speedCurve.ts` (~190 lines) — `warpTime()`, `calculateWarpedDuration()`, `validateWarpFunction()`, `analyzeWarpCurve()`
- Internal inverse easing via binary search with WeakMap caching, monotonicity checking

#### Feature 3: Grid Splitter Utility
- `lib/gridSplitter.ts` (~380 lines) — Auto-detect contact sheet grids from pixel data
- `detectGrid()`, `getGridCandidates()`, `splitImage()`, `detectAndSplitGrid()`, `splitWithDimensions()`

#### Feature 4: Node Dimensions Utility
- `lib/nodeDimensions.ts` (~120 lines) — `calculateNodeSize()`, `calculateNodeSizePreservingHeight()`
- Min/max constraints: 200-500w, 200-600h, 100px chrome

#### Feature 5: Deduplicated Fetch
- `lib/deduplicatedFetch.ts` (~100 lines) — Drop-in `fetch` replacement with 5s TTL cache and in-flight dedup

#### Feature 6: NotificationToast
- `components/NotificationToast.tsx` (~100 lines) — Renders existing uiStore notifications with type-based colors, lucide-react icons, copy-to-clipboard

#### Feature 7: ReferenceEdge
- `canvas/ReferenceEdge.tsx` (~95 lines) — Dimmed dashed edge with SVG gradient, selection-aware opacity, curved bezier paths

#### Feature 8: useCommentNavigation
- `hooks/useCommentNavigation.ts` (~70 lines) — Circular prev/next navigation between commented nodes

#### Feature 9: GlobalImageHistory
- `components/GlobalImageHistory.tsx` (~300 lines) — Mac-style fan-out image history drawer with drag-to-canvas
- Store additions: `ImageHistoryItem` type, `globalImageHistory[]` state (capped at 100), `addToGlobalHistory()`/`clearGlobalHistory()` actions

### Files created

| File | Feature | ~Lines |
|------|---------|--------|
| `src/lib/speedCurve.ts` | Speed curve time-warping | 190 |
| `src/lib/gridSplitter.ts` | Contact sheet grid detection | 380 |
| `src/lib/nodeDimensions.ts` | Node size from aspect ratio | 120 |
| `src/lib/deduplicatedFetch.ts` | Fetch request deduplication | 100 |
| `src/components/NotificationToast.tsx` | Toast notification UI | 100 |
| `src/canvas/ReferenceEdge.tsx` | Dimmed reference edge | 95 |
| `src/hooks/useCommentNavigation.ts` | Comment nav hook | 70 |
| `src/components/GlobalImageHistory.tsx` | Image history drawer | 300 |

### Files modified

| File | Changes |
|------|---------|
| `src/lib/easing.ts` | Extended with parametric functions + bezier maps |
| `src/stores/workflow/types.ts` | Added `ImageHistoryItem`, `globalImageHistory` state, `ImageHistoryActions` |
| `src/stores/workflow/slices/nodeSlice.ts` | Implemented `addToGlobalHistory`, `clearGlobalHistory` |
| `src/stores/workflow/workflowStore.ts` | Added `globalImageHistory: []` initial state |
| `src/canvas/WorkflowCanvas.tsx` | Added `reference` edge type, history-image drop handler, renders NotificationToast + GlobalImageHistory |
| `src/canvas/index.ts` | Exports ReferenceEdge |
| `src/lib/index.ts` | Exports all new lib modules |
| `src/hooks/index.ts` | Exports useCommentNavigation |
| `src/index.ts` | Package-level exports for new components, hooks, types |

### Decisions

- **Decision:** Added history actions to `nodeSlice.ts` rather than creating a separate `historySlice.ts`.
  - **Rationale:** Simpler, follows existing pattern of grouping related actions.

- **Decision:** Used renamed exports (`getImageDimensionsFromDataUrl`, `getVideoDimensionsFromUrl`) in lib barrel.
  - **Rationale:** Avoids naming conflict with existing `getImageDimensions` from `./media`.

- **Decision:** Kept `EASING_PRESETS` Record<EasingPreset, CubicBezier> unchanged, added separate `EASING_BEZIER_MAP`.
  - **Rationale:** Backward compat — EasingPreset is a union type in @genfeedai/types.

- **Decision:** Replaced `console.warn` in `getEasingFunction` with silent fallback to `easing.linear`.
  - **Rationale:** Project rules prohibit console.log/warn.

### Skipped (per plan)

- `imageStorage.ts` — uses server endpoint + Node.js crypto
- `video-encoding.ts` — depends on `mediabunny` types
- `SplitGridSettingsModal` — tied to SplitGrid→NanoBanana workflow pattern

### Verification

- 28/28 easing tests pass
- 211/211 total tests pass
